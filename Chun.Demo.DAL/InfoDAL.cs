...
namespace Chun.Demo.DAL
{
    public class InfoDal
    {
        ...
        {
            var newFileStatusId = (fileStatusId == 0 ? new int?[] { 0 } : 
                (fileStatusId == 1 ? new int?[] { 1 } : 
                (fileStatusId == 2 ? new int?[] { 2 } :
                (fileStatusId == 3 ? new int?[] { 0,2 } :
                new int?[] { 0, 1, 2 }))));
            Expression<Func<filepath, bool>> func = (c => c.file_Type_id == fileTypeId && newFileStatusId.Contains(c.file_status_id));
        ...
        public static void UpdatefilePathByLinq (string file_Path, int file_Type_id, int file_status_id )
        {
           // new BaseDataQuery<filepath>( ).Update(filepath);
            string sql  = "update filepath set file_status_id = {0} ,file_updatetime = {1} where  file_path=  {2} and file_type_id = {3}";
           
            new BaseDataQuery<filepath>( ).ExecuteSql(sql,new object[] { file_status_id, DateTime.Now,file_Path,file_Type_id });
        ...
        public static void InsertfilePath ( string path, string innerTxt, int fileType, int fileStatus, string fileParentPath )
        {
            string sql = "insert into FilePath (file_Path,file_innerTxt,file_Type_id,file_status_id,file_CreateTime,file_parent_path) values ('" + path + "'" + ",'" + innerTxt + "','" + fileType + "','" + fileStatus + "','" + DateTime.Now + "','" + fileParentPath + "')";
            ISql<SqlCommand, SqlConnection> mysql = new MsSql( );
            mysql.run(sql, mysql.getInsert);
        }
        /// <summary>
        ...
        {
            string sql = "select distinct file_path from filepath where file_type_id= " + type.ToString( ) + " and file_status_id in ( " + (fileStatus == 0 ? "0,2" : (fileStatus == 1 ? "1" : "0,1,2")) + ")";
            ISql<SqlCommand, SqlConnection> mysql = new MsSql( );
        ...
        public static void UpdatefilePath ( string path, int filetype, int file_status )
        {
            ...
            string sql = "update filepath set file_status_id =" + file_status + " ,file_updatetime = '" + DateTime.Now + "' where  file_path=  '" + path + "' and file_type_id = " + filetype;
            ISql<SqlCommand, SqlConnection> mysql = new MsSql( );
